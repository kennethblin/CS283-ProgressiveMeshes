...
stack < vector< vector<float> > > reverseErrors;
...
void makeQuadric() {
    ...
    for (int i = 0; i < allFaces.size(); i++) {
        glm::vec3 a(allVerts[allFaces[i][1]][0],allVerts[allFaces[i][1]][1],allVerts[allFaces[i][1]][2]);
        glm::vec3 b(allVerts[allFaces[i][2]][0],allVerts[allFaces[i][2]][1],allVerts[allFaces[i][2]][2]);
        glm::vec3 c(allVerts[allFaces[i][3]][0],allVerts[allFaces[i][3]][1],allVerts[allFaces[i][3]][2]);
        glm::vec3 ab = b-a;
        glm::vec3 bc = c-b;
        for (int j=1; j<4; j++) {
                glm::vec3 v(allVerts[allFaces[i][j]][0],allVerts[allFaces[i][j]][1],allVerts[allFaces[i][j]][2]);
                float d = -glm::dot(a, v);
                ...
                glm::mat4 newvalue;
                if (errorMatrix.find(allFaces[i][j]) != errorMatrix.end()) {
                        newvalue = errorMatrix[allFaces[i][j]]+k;
                } else {
                        newvalue = k;
                }
                errorMatrix[allFaces[i][j]] = newvalue;
        }
    }
}
...
void changeEdge(int stay, int out) {
    ...
    for (int k = 0; k < removal.size(); k++) {
        allEdges.erase(allEdges.begin() + removal[k]);
    }
}
...
void changeFace(int stay, int out) {
    ...
    for (int k = 0; k < allFaces.size(); k++) {
        bool isFake = false;
        for (int l = 0; l < removal.size(); l++) {
            if (k == removal[l]){
                isFake = true;
            }
        }
        if (!isFake) {
            newFaces.push_back(allFaces[k]);
        }
    }
    allFaces = newFaces;
    changeEdge2();
...
void reverseEdgeColl() {
    ...
    //THIS IS TO PUT BACK ERROR METRICS
    if (!(reverseErrors.empty())) {
        pairOrder = reverseErrors.top();
        reverseErrors.pop();
    }
}
